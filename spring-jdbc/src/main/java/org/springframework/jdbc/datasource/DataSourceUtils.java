...
public abstract class DataSourceUtils {
	...
	/**
	 * 获取 数据库 连接，考虑到多种情况，实际实现方法在：{@link #doGetConnection(DataSource)}
	 * <p>
	 * 从给定的 数据源 中获取连接。将 SQLExceptions 转换为未检查的 通用数据访问异常 的Spring层次结构，
	 * 从而简化代码的调用并使抛出的 异常 更有意义。
	 * <p>知道绑定到当前线程的相应连接，例如在使用 DataSourceTransactionManager 时.</p>
	 * 如果事务同步处于活动状态（例如在事务内运行），则将连接绑定到线程.
	 * <p>
	 ...
	 * 获取 数据库 连接的实际操作 函数。
	 * <p>
	 * 考虑到 事务方面 的处理，基于事务处理的特殊性，Spring 需要保证线程中的 数据库操作 都是使用了同一个 事务连接。
	 * <p>
	...
	public static Connection doGetConnection(DataSource dataSource) throws SQLException {
		...
		// 包装 JDBC 连接的 资源持有者 对象。
		ConnectionHolder conHolder = (ConnectionHolder) TransactionSynchronizationManager.getResource(dataSource);
		// JDBC 连接资源 的持有者 不为空，并且 有链接； 或者 资源持有者 与线程事务同步。
		if (conHolder != null && (conHolder.hasConnection() || conHolder.isSynchronizedWithTransaction())) {
			...
			// 直接从 JDBC 连接资源 池 中获取 连接。
			return conHolder.getConnection();
		}
		// Else we either got no holder or an empty thread-bound holder here.
		// 否则，我们要么没有 JDBC 连接资源 池，要么是一个 空的 线程绑定 JDBC 连接资源 池。

		logger.debug("Fetching JDBC Connection from DataSource");
		// 从 DataSource 中 获取 JDBC 连接
	 ...
	 * 释放 数据库的连接 资源。
	 * <p>
	 * 如果给定的连接没有被外部管理（也就是说，没有绑定到线程），请关闭 从给定的数据源 中 获取的 该连接。
	 * <p>
	...
	public static void doReleaseConnection(@Nullable Connection con, @Nullable DataSource dataSource) throws SQLException {
		...
		if (dataSource != null) {
			// 当前线程存在事务 的情况下，说明存在共用数据库连接，直接使用 ConnectionHolder中的 released 方法进行 连接数减一，而不是真正的释放连接。
			ConnectionHolder conHolder = (ConnectionHolder) TransactionSynchronizationManager.getResource(dataSource);
