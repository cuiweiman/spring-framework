...
public abstract class AbstractPlatformTransactionManager implements PlatformTransactionManager, Serializable {
	 ...
	 * 获取 事务
	 * <p>
	 * 1. 获取事务。创建对应的事务实例：{@link org.springframework.jdbc.datasource.DataSourceTransactionManager#doGetTransaction()}
	 * 2. 如果当前线程存在事务，则转向 嵌套事务的处理{@link #handleExistingTransaction(org.springframework.transaction.TransactionDefinition, java.lang.Object, boolean)}。
	 * 3. 事务超时设置验证；
	 * 4. 事务 propagationBehavior 属性的设置验证。
	 * 5. 构建 DefaultTransactionStatus。
	 * 6. 完善 transaction，包括设置 ConnectionHolder、隔离界别、timeout，如果是新连接，那么绑定到当前线程。
	 * 对于 隔离界别、timeout 等功能的设置 是由 Spring 委托给 数据库连接 实现的。而对于数据库连接的设置就在
	 * {@link org.springframework.jdbc.datasource.DataSourceTransactionManager#doBegin(java.lang.Object, org.springframework.transaction.TransactionDefinition)} 函数中
	 * <p>
	 ...
	 * @see #prepareSynchronization 将事务信息 记录在当前 线程中
	 * @see #handleExistingTransaction 当前线程中，已经有事务存在时的处理
	 */
	@Override
	public final TransactionStatus getTransaction(@Nullable TransactionDefinition definition)
		...
		// 判断当期线程是否存在事务，判断依据为 当前线程记录的连接不为空，且连接中（connectionHolder）中的 transactionActive 属性不为空。
		...
		// 当前线程 不包含事务，而 propagationBehavior 被声明为 PROPAGATION_MANDATORY，那么抛出异常。
		if (def.getPropagationBehavior() == TransactionDefinition.PROPAGATION_MANDATORY) {
			...
			// 这配置了 propagationBehavior 属性这三个属性值，那么在当前线程没有事务时，需要 新建事务。
	...
	private TransactionStatus startTransaction(TransactionDefinition definition, Object transaction,
		...
		/*
		构造 transaction，包括设置 ConnectionHolder、隔离级别、timeout，如果是新连接，那么绑定到当前线程。
		 */
	...
	/**
	 * 当前线程 中 已存在 事务 时，根据 配置的 传播规则 进行处理。
	 * <p>
	 * PROPAGATION_REQUIRES_NEW：当前方法必须在它自己事务中运行，要启动一个新的事务。如果现在有事务正在运行的话，那么在这个方法运行期间，
	 * 现有事务将会被挂起。在Spring 中使用 {@link AbstractPlatformTransactionManager#suspend(java.lang.Object)} 方法将原事务挂起，在当前事务执行结束后，再将事务还原。
	 * <p>
	 * PROPAGATION_NESTED：如果当前线程中有事务在运行，则这个方法应该运行在嵌套事务中，被嵌套的事务可以独立于封装事务 进行回滚或者提交，
	 * 如果封装事务不存在，此时行为就像  PROPAGATION_REQUIRES_NEW 。对于嵌入式事务的处理，Spring 中主要考虑两种方式：
	 * 1. Spring 允许嵌入事务时，首先设置保存点的方式作为异常处理的回滚。
	 * 2. 其他方式，比如JTA，无法使用保存点回滚，那么处理方式与 PROPAGATION_REQUIRES_NEW 相同。出现异常，由Spring 事务异常处理机制完成后续操作。
	 *
	 * <p>
	...
	private TransactionStatus handleExistingTransaction(
		...
		if (definition.getPropagationBehavior() == TransactionDefinition.PROPAGATION_NESTED) {
			...
			if (useSavepointForNestedTransaction()) {
				...
				// 如果没有可以使用保存点的方式控制事务回滚，那么在嵌入式 事务的 建立初始 建立 保存点。
				...
				// 有些情况 不能使用 保存点操作，比如 JTA，那么创建新事务。
	...
	/**
	 * 如果在事务执行之前，有事务被挂起，那么当前事务执行结束后，要将挂起的事务恢复执行。
	 ...
	 * @see #doCommit
	 * @see #rollback
	 * @see #processCommit(DefaultTransactionStatus) 事务提交流程
	 */
	@Override
	public final void commit(TransactionStatus status) throws TransactionException {
		...
		// 如果在事务链中，已经被标记为回滚，那么不会尝试提交事务，直接回滚。
	...
	/**
	 * 事务并不是直接提交的，需要符合提交的条件如下：
	 * 1. 当事务状态中有保存点信息的话，便不会去提交事务；
	 * 2. 当事务不是新事务的时候，也不会去执行提交事务操作。
	 * <p>
	 ...
	 * @see #cleanupAfterCompletion(org.springframework.transaction.support.DefaultTransactionStatus)
	 */
	 ...
	 * 清空记录的 资源，并将挂起的资源恢复
	 * <p>
	 * 回滚结束后，不论是否回滚成功，都必须做的收尾工作。
	 * <p>
	 * 1. 设置状态是对事务信息作完成标识，以避免重复调用。
