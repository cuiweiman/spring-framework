...
public abstract class TransactionAspectSupport implements BeanFactoryAware, InitializingBean {
	...
	/**
	 * 用于基于通知的子类的常规委托，委托给该类上的其他几个模板方法。能够处理 CallbackPreferringPlatformTransactionManager
	 * 以及常规 PlatformTransactionManager 实现和被动返回类型 ReactiveTransactionManager 的实现。
	 * <p>
	 * Spring 支持两种事务处理方式，声明式事务处理 和 编程式事务处理，目前比较流行声明式事务处理，因为其对应用代码的影响最小, 并且最符合一个非 侵入型轻量级容器的理想.
	 * <p>
	 * 1. 获取事务的属性。
	 * 2. 加载并配置 TransactionManager 。
	 * 3. 不同的事务处理使用不同的逻辑。
	 * 4. 在目标方法执行前获取事务并收集事务信息。事务信息（TransactionInfo）与事务属性（TransactionAttribute）不同。
	 * 5. 执行目标方法。
	 * 6. 一旦出现异常，尝试异常处理。Spring默认只对 RuntimeException 回滚。
	 * 7. 提交事务前将事务信息清除。
	 * 8. 提交事务。
	 * <p>
	...
	protected Object invokeWithinTransaction(Method method, @Nullable Class<?> targetClass,
		...
		if (txAttr == null || !(ptm instanceof CallbackPreferringPlatformTransactionManager)) {
			...
			// ★★ 创建事务。创建 TransactionInfo。
			TransactionInfo txInfo = createTransactionIfNecessary(ptm, txAttr, joinpointIdentification);

			Object retVal;
			try {
				// This is an around advice: Invoke the next interceptor in the chain. 这是一个 around 的建议：调用链中的下一个拦截器。
	 ...
	 * 创建事务信息
	 * <p>
	 * 1. 使用 DelegatingTransactionAttribute 封装参数 TransactionAttribute 实例（主要用于数据承载），以便提供更多功能。
	 * 2. 获取事务。
	 * 3. 构建事务信息，返回 TransactionInfo。
	 * <p>
	 ...
	 * @see #prepareTransactionInfo(PlatformTransactionManager, TransactionAttribute, String, TransactionStatus)
	 */
	...
	protected TransactionInfo createTransactionIfNecessary(@Nullable PlatformTransactionManager tm,
		...
		// 若没有指定名称，则使用方法唯一标识，并使用 DelegatingTransactionAttribute 封装 txAttr
		...
		// 根据指定的属性 与 status 创建一个 TransactionInfo
		return prepareTransactionInfo(tm, txAttr, joinpointIdentification, status);
	}

	/**
	 * 当已经建立 事务连接并完成了事务信息的提取后，我们需要将 所有的事务信息 统一记录在 TransactionInfo 实例中，
	 * 包括目标方法开始前的所有状态信息，一旦事务执行失败，Spring 会通过 TransactionInfo 类型的实例中的信息进行回滚等事务操作。
	 * <p>
	 * 根据指定的属性 与 status 创建一个 TransactionInfo。
