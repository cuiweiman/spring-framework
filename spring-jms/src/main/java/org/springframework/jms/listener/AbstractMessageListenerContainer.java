...
public abstract class AbstractMessageListenerContainer extends AbstractJmsListeningContainer
	...
	/**
	 * 激活 指定的 监听器，之后再对事务进行提交或者回滚（如果有需要，即有配置）。
	 * Execute the specified listener,
	 * committing or rolling back the transaction afterwards (if necessary).
	 *
	 * @param session the JMS Session to operate on
	 ...
	 * @see #invokeListener 激活监听器
	 * @see #commitIfNecessary 若配置了事务，则进行事务提交
	 * @see #rollbackOnExceptionIfNecessary 若配置了事务，出现异常时进行事务回滚
	 ...
	 * @throws JMSException if thrown by JMS API methods
	 * @see #setMessageListener
	 * @see #doInvokeListener(SessionAwareMessageListener, Session, Message)激活监听器
	 */
