...
/**
 * 主要职责是 维护了 链接调用 的计数器，记录着当前调用链接的位置，以便 拦截器链 可以有序地 进行下去。
 * <p>
...
public class ReflectiveMethodInvocation implements ProxyMethodInvocation, Cloneable {
	...
	/**
	 * 实现拦截器链 方法的逐一调用。
	 * 使得 前置增强 在目标方法执行前 调用，后置增强 在目标方法执行后 调用。
	 *
	 * @return 执行结果
	...
	public Object proceed() throws Throwable {
		...
		if (interceptorOrInterceptionAdvice instanceof InterceptorAndDynamicMethodMatcher) {
			...
			/*
			普通拦截器，直接调用 拦截器方法执行。普通拦截器比如：
			ExposeInvocationHandler、DelegatePerTargetObjectIntroductionInterceptor、MethodBeforeAdviceInterceptor、
			AspectJAroundAdvice、AspectJAfterAdvice。
			 */
			// It's an interceptor, so we just invoke it: The pointcut will have
			// been evaluated statically before this object was constructed.
			// 将 this 作为参数传递 以保证当前实例中 调用链 的执行
