...
/**
 * 实现BeanDefinition接口的抽象类。{@see DefaultBeanDefinitionDocumentReader#processBeanDefinition}解析完XML中配置的
 * Bean Definition 信息后，即完成了 XML文档 到 {@link GenericBeanDefinition}  的转换后，XML文档中所有的配置都可以在
 * GenericBeanDefinition 实例类中 找到对应的配置。
 * GenericBeanDefinition 是 AbstractBeanDefinition 的子类实现，大部分的通用属性都保存在了 AbstractBeanDefinition 中。
 * <p>
...
public abstract class AbstractBeanDefinition extends BeanMetadataAttributeAccessor
	...
	/**
	 * Bean 的作用范围，对应 <bean> 标签 中的 scope 属性
	 */
	@Nullable
	private String scope = SCOPE_DEFAULT;

	/**
	 * Bean 是否是抽象类，对应 <bean> 标签 中的 abstract 属性
	 */
	private boolean abstractFlag = false;

	/**
	 * Bean 是否 延迟加载，对应 <bean> 标签 中的 lazy-init 属性
	 */
	@Nullable
	private Boolean lazyInit;

	/**
	 * Bean 自动注入模式，对应 <bean> 标签 中的 autowire 属性，值可以是 byName,byType,no,constructor,default
	 */
	private int autowireMode = AUTOWIRE_NO;

	/**
	 * Bean 一来检查，Spring 3.0 后弃用这个属性
	 */
	private int dependencyCheck = DEPENDENCY_CHECK_NONE;

	/**
	 * 表示 该Bean 的实例化 依赖于另一个 Bean，需要先将另一个 Bean 实例化。对应 <bean> 标签 中的 depend-on 属性
	 */
	@Nullable
	private String[] dependsOn;

	/**
	 * autowire-candidate 属性设置为 false，此时容器在查找自动装配对象时，该对象不会被自动注入，
	 * 但是 该 Bean 本身还是可以使用自动装配来注入其它的 Bean。
	 * 常见用于 一个接口有两个Bean实现类，其中一个Bean实现类中配置了autowire-candidate=false，那么在实例化 接口时，会使用另一个Bean实现类实例化。
	 * 对应 <bean> 标签 中的 autowire-candidate 属性
	 */
	private boolean autowireCandidate = true;

	/**
	 * 自动装配时，若有多个 Bean 候选者，primary=true的 Bean 将作为首选这。对应 <bean> 标签 中的 primary 属性
	 */
	private boolean primary = false;

	/**
	 * 多有一个接口有多个实现类，在对接口类型进行自动装配时，可以使用 qualifier 声明需要注入的具体实现类的名称
	 * 记录 Qualifier，对应 <bean> 标签 中的 qualifier 属性。
	 */
	...
	/**
	 * 允许 访问非公开的 构造器和方法，程序设置
	 */
	private boolean nonPublicAccessAllowed = true;

	/**
	 * 是否以一种宽松的模式 解析构造函数，默认为 true。如果为false，则在如下情况 会抛出异常，因为Spring 无法准确定位哪个构造函数，程序设置
	 * interface ITest{}
	 * <p>
	...
	/**
	 * 对应 <bean> 标签 下的 factory-bean 属性。用法：
	 * ```xml
	...
	/**
	 * 对应 <bean> 标签下的 factory-method 属性。
	 */
	@Nullable
	private String factoryMethodName;

	/**
	 * 记录 构造函数 注入的 属性，对应 <bean> 标签 下的 <constructor-arg> 子标签
	 */
	@Nullable
	...
	/**
	 * 方法重写的 持有者 ，对应 <bean> 标签 下的 <lookup-method>、<replaced-method> 子标签
	 */
	private MethodOverrides methodOverrides = new MethodOverrides();

	/**
	 * 初始化方法，对应 <bean> 标签 下的 init-method 属性
	 */
	@Nullable
	private String initMethodName;

	/**
	 * 销毁方法，对应 <bean> 标签 下的 destroy-method 属性
	 */
	@Nullable
	...
	/**
	 * 是否是 用户定义的 而不是 应用程序本身 定义的，创建 AOP 时为 true，程序设置
	 */
	 ...
	 * 定义这个 Bean 的应用。程序设置。
	 * APPLICATION：用户使用；
	 * ROLE_SUPPORT：某些复杂配置的 一部分；
	 * ROLE_INFRASTRUCTURE：完全内部使用，与用户无关
	 */
	private int role = BeanDefinition.ROLE_APPLICATION;

	/**
	 * Bean的 描述信息。对应 <bean> 标签 下的 <description> 标签
	 */
	@Nullable
	private String description;

	/**
	 * 这个 Bean Definition 的资源
	 */
	@Nullable
