...
public abstract class AbstractSingleBeanDefinitionParser extends AbstractBeanDefinitionParser {
	...
	protected final AbstractBeanDefinition parseInternal(Element element, ParserContext parserContext) {
		...
		// 获取自定义标签中的 class，此时会调用 自定义解析器如 UserBeanDefinitionParser 中的 getBeanClass 方法
		// 案例在：{@see https://github.com/cuiweiman/wang-wen-jun#com.wang.think.customeralias}
		Class<?> beanClass = getBeanClass(element);
		if (beanClass != null) {
			builder.getRawBeanDefinition().setBeanClass(beanClass);
		} else {
			// 若 自定义的解析器没有重写 getBeanClass 方法，则尝试检查子类是否有重写 getBeanClassName 方法
		...
		if (containingBd != null) {
			...
			// 通过委派给为{@link Element}注册的{@link BeanDefinitionParser}来解析提供的{@link Element}。
